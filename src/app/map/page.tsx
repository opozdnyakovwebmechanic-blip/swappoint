...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/items").then((response) => response.ok ? response.json() : { items: [] }).then(({ items }) => setListings(items.map((item: { id: string; title: string; pointsPrice: number; district: string; latitude: number | null; longitude: number | null }) => ({ id: item.id, title: item.title, points: item.pointsPrice, district: item.district, latitude: item.latitude, longitude: item.longitude })))).catch(() => undefined); }, []);
